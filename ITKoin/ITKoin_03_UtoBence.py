...
class ITKoin:
    def __init__(self):
        ...
        #        self.my_publickey
        #        self.my_id
        self.ITKoin_users = ['csmkey_03_id.txt',
                             'csmkey_04_id.txt']  # Ez egy lista, ahová s résztvevők id-jait tartalmazó file-neveket kell felsorolni
        self.initial_csaposhi_offering = 100

    def generate_rsa_key(self,
                         filename):  # a filenév tövével kell meghívni és három file-t generál: a privát és publikus kulcsoknak, ill. az ID-nak
        ...
        f.close()
        publickey_string = publickey.export_key().decode(
            'ascii')  # bináris stringet karakter stringgé konvertáljuk, hogy a json.dumps működjön rajta
        recipient_id_filename = filename + '_id.txt'
        f = open(recipient_id_filename, 'wb')
        f.write(self.create_hashhexvalue(publickey_string).encode(
            'ascii'))  # a hexa string hash értéket bináris stringgé konvertáljuk a file-ba íráshoz
        f.close()
        return
    ...
    def load_my_private_key(self):
        ...
        publickey_string = self.my_publickey.export_key().decode(
            'ascii')  # bináris stringet karakter stringgé konvertáljuk, hogy a json.dumps működjön rajta
        self.my_id = self.create_hashhexvalue(
            publickey_string)  # a hexa string hash értéket bináris stringgé konvertáljuk a file-ba íráshoz
        pprint(self.my_id)
        return
    ...
    def load_chain(self):
        ...
        valids = []
        for x in self.chain:
            prev_header_hash = x['block_header']['previous_block_header_hash']
            if prev_header_hash == 'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff':
                valids.append(x)
                continue
            for y in self.chain:
                header_hash = self.create_hashhexvalue(y['block_header'])
                if prev_header_hash == header_hash:
                    valids.append(x)
                    break
        for x in valids:
            transaction_hash = self.create_hashhexvalue(x['transactions'])
            if x['block_header']['transactions_hash'] != transaction_hash:
                valids.remove(x)
                continue

            block_header_hash = self.create_hashhexvalue(x['block_header'])
        ...
        while len(self.pending_transactions) != 0:
            ...
            try:
                ...
                for input in transaction['inputs']:
                    sumIn += input[2]
                    self.verify_signature(input[0], input[3], RSA.import_key(input[4]))
                    for output in self.my_unspent_outputs:
                        if output[0] == input[0] and output[1] == input[1] and output[2] == input[2]:
                            valid = True
            except ValueError:
                continue

            for output in transaction['outputs']:
                sumOut += output['csaposhi']

            if sumOut != sumIn:
                valid = False

            if valid:  # itt validálni kellene az adott tranzakciót, a nem érvényeseket eldobja, de nem áll le
            ...
            for transaction in block['transactions']:
                for output in transaction['outputs']:
                    self.unspent_outputs.append(
                        [transaction['txid'], transaction['outputs'].index(output), output['csaposhi']])
                    if output['recipient'] == self.my_id:
                        self.my_unspent_outputs.append(
                            [transaction['txid'], transaction['outputs'].index(output), output['csaposhi']])
                for input in transaction['inputs']:
    ...
    def generate_first_block(self):
        ...
        while len(self.ITKoin_users) > 0:
            recipient_id = self.load_id(
                self.ITKoin_users.pop())  # előveszi a következő id file nevét és beolvassa az id-t
            ...
            transaction['txid'] = self.create_hashhexvalue(
                transaction)  # a tranzakció lenyomata lesz az azonosítója egyben
